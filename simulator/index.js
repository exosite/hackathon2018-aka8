...
log4js.configure({
    appenders: {
        out: {type: 'console'},
        default: {type: 'dateFile', filename: 'logs/ec2_hamv', "pattern": "-yyyy-MM-dd.log", alwaysIncludePattern: true},
    },
    categories: {
...
async function report(id) {
    ...
    while (i <= PUBLISH_TIMES) {
        try {
            let ret = await async_client.publish('$resource/states', `{"H04": ${i}, "H01": ${i}, "H03": ${i}}`, {qos:1})
            // logger.debug(`${id} set H04 as ${i}`)
            logger.debug(`{"H04": ${i}, "H01": ${i}, "H03": ${i}}`)
